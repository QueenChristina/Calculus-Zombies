...
credit_sign = pygame.image.load(os.path.join('Stuff', 'Credit.png'))
credit_sign = pygame.transform.scale2x(credit_sign)
credit_sign_rect = credit_sign.get_rect()
...
def do_menu():
    ...
    Sprite.DISPLAY.blit(copyrt, (380,650))
    ...
    elif credit_sign_rect.collidepoint(Var.mousePOS):
        pygame.draw.rect(Sprite.DISPLAY, (220, 80, 100), credit_sign_rect, 0)
    ...
    Sprite.DISPLAY.blit(credit_sign, (650,550))
    credit_sign_rect.topleft = (650,550)
       ...
       # Place ALL button/mouse click detecting in here to that no accidental clicking before reading takes place 
        if Var.mouseDOWN == True:
            if class_sign_rect.collidepoint(Var.mouseDOWNPOS):
                ...
                Var.calculator = True
                Var.menu = False
            elif credit_sign_rect.collidepoint(Var.mouseDOWNPOS):
                Var.set_up_credit = True
                Var.credit = True
                Var.menu = False
...
save = pygame.image.load(os.path.join('Stuff', 'Save.png'))
save = pygame.transform.scale2x(save)

magic = pygame.image.load(os.path.join('Stuff', 'Magic.png'))
magic = pygame.transform.scale2x(magic)

# to include more lessons, change number to last picture in folder
# also chang Images.listimages as list not finished
learn = Images.listimages('Learn', 20)

intro = [hi, intro, joke, joke2, know, drool, anyway]

end = [magic, save, anyway, test, late, eat]
...
##############################################################################################################################################
cred_back = pygame.image.load(os.path.join('Stuff', 'Menu.png'))
cred_back = pygame.transform.scale(cred_back, (Sprite.DISPLAY_X, Sprite.DISPLAY_Y))
cred = pygame.image.load(os.path.join('Stuff', 'Credit Page.png'))
cred = pygame.transform.scale2x(cred)
def credit():
    if Var.set_up_credit == True:
        Var.mouseDOWN = False
        Var.set_up_credit = False

    Sprite.DISPLAY.blit(cred_back, (0,0))
    Sprite.DISPLAY.blit(cred, (100,50))
